...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($themeToDelete)) {
        ...
        $query = 'DELETE FROM themes WHERE theme_name = ?';
        $stmt = mysqli_prepare($conn, $query);
        ...
        if (mysqli_stmt_affected_rows($stmt) > 0) {
            ...
            if (count($files) > 0) {
                ...
                $_SESSION['message'] = "Theme file not found on the server.";
            }
        } else {
            $_SESSION['message'] = "Failed to delete theme from the database.";
        }

        mysqli_stmt_close($stmt);
    } else {
        $_SESSION['message'] = "No theme selected.";
    }

    mysqli_close($conn);
    header('Location: delete_theme.php'); // Redirect to avoid resubmission
    exit();
}
?>
	<!DOCTYPE html>
...
<!-- Navbar -->
    ...
    <div class="options-container">
        ...
        <!-- Delete Theme Form -->
        <form action="delete_theme.php" method="post" class="form-group">
            <div class="mb-3">
            ...
            <button type="submit" class="btn btn-primary"><i class="bi bi-trash"></i> Delete Theme</button>
        </form>
    </div>
